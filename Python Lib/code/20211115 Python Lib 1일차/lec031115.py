...
# (3) 레코드 삽입
# 레코드삽입 1(필드명 생략방식)
# 필드값이 AUTO_INCREMENT인 경우 Null 값으로 지정 
'''
sql = "INSERT INTO 테이블명 VALUES(값1, 값2)"(레코드 삽입)
cursor.execute(sql)
conn.commit()'''
# sql='INSERT INTO bookTbl VALUES (NULL,"파이썬 완전정복","박파이",600,30000)'
# cursor.execute(sql)
# conn.commit() # 데이터 베이스 닫아줌 
...
# cursor.execute(sql,('이것이 오라클이다.','박오라',400,25000))
# cursor.execute(sql,('이것이 파이썬이다.','박파이',300,20000))
# conn.commit()
...
conn.commit
# (5) 레코드 수정
'''sql = "UPDATE 테이블명 SET 필드1=%s WHERE 필드 2 = %s"
cursor.execute(sql, (값1, 값2...))
conn.commit()
'''
