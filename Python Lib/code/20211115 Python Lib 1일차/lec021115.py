...
# 데이타 변수명 = cursor. fetchall() -2차원 튜플/ cursor.fetchone()- 1차원 튜플  / cursor.fetchmany(n)-2차원 튜플

sql = 'select * from city limit 20';
cursor.execute(sql)
cityData = cursor.fetchall() # 2차원 튜플
#cityData = cursor.fetchone() # 1차원 튜플 - 가장 많이 사용
#cityData = cursor.fetchmany(10)
...
#(5) csv 저장
df.to_csv('output/city10.csv',index=False)
